<form class="form container <?=!empty($error) ? 'form--invalid' : '' ?>" action="login.php" method="post"> <!-- form--invalid -->
    ...
    <div class="form__item <?=isset($error['email']) ? 'form__item--invalid' : '' ?>"> <!-- form__item--invalid -->
        ...
        <input id="email" type="text" name="email" value="<?=$user_enter['email'] ?? ''; ?>" placeholder="Введите e-mail">
    ...
    <div class="form__item form__item--last <?=isset($error['password']) ? 'form__item--invalid' : '' ?>">
        ...
        <input id="password" type="password" name="password" value="<?=$user_enter['password'] ?? ''; ?>" placeholder="Введите пароль">
